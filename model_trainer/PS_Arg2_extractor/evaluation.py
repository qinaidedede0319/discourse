...
def get_Arg2_acc(parse_dict, relations, dev_feature_path, dev_classifier_out_path):
    ...
    for feature_line, predicted in zip(feature_list, predicted_list):
        ...
        clause_indices = map(int, comment.split("|")[-1].strip().split(" "))
        if relationID not in dict_relationID_to_predicted_clause_indices:
            dict_relationID_to_predicted_clause_indices[relationID] = []
        dict_relationID_to_predicted_clause_indices[relationID].append((clause_indices, predicted))
    ...
    for relationID in dict_relationID_to_predicted_clause_indices:
        ...
        for clause_indices, pred in predicted_clause_indices:# 合并是请补上标点符号，亲！
            if pred == "1":
                Arg_list.extend(clause_indices)

        Arg_list = sorted(Arg_list)

        Arg_list = merge_for_PS_Arg2(parse_dict, DocID, sent_index, Arg_list)
...
#[1,2,4,5,6,7]
def merge_for_PS_Arg2(parse_dict, DocID, sent_index, Arg_list):
    punctuation = """!"#&'*+,-..../:;<=>?@[\]^_`|~--""" + "``" + "''"
    if len(Arg_list) <= 1:
        return Arg_list
    temp = []
