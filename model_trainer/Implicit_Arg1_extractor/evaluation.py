...
#[1,2,4,5,6,7]
def merge_Arg(parse_dict, DocID, sent_index, Arg_list):
    punctuation = """!"#&'*+,-..../:;<=>?@[\]^_`|~--""" + "``" + "''"
    if len(Arg_list) <= 1:
        return Arg_list
    temp = []
        ...
        if i <= len(Arg_list) - 2:
            ...
            if next_item - item > 1:
                ...
                for j in range(item + 1, next_item):
                    w = parse_dict[DocID]["sentences"][sent_index]["words"][j][0]
                    if w not in punctuation and w not in ["-LCB-", "-LRB-", "-RRB-", "-RCB-"]:
                        flag = 0
                        break
                if flag == 1:#都是标点，补齐
