import sys
sys.path.append("../../")
from model_trainer.Implicit_Arg2_extractor.evaluation import get_Arg2_acc
from model_trainer.Implicit_Arg2_extractor.make_feature_file import Make_feature_file
from model_trainer.mallet_classifier import *
import json, pickle
from model_trainer.Implicit_Arg2_extractor.feature_functions import all_features
...
class Trainer(object):
    ...
    def make_feature_file(self, train_parse_dict, dev_parse_dict, train_clauseArguments, dev_clauseArguments):
        ...
        Make_feature_file(train_parse_dict, train_clauseArguments, self.feature_function_list, self.train_feature_path)
        print("make dev feature_file file ...")
        Make_feature_file(dev_parse_dict, dev_clauseArguments, self.feature_function_list, self.dev_feature_path)
...
if __name__ == "__main__":
    ...
    dev_classifier_out_path = config.IMPLICIT_ARG2_DEV_OUTPUT_PATH
    ...
    with \
        ...
        open(config.DATA_PATH + "implicit_arg2_vs_null/train/implicit_arg2_vs_null_clauseArguments.pkl", "rb") as train_pkl_file, \
        open(config.DATA_PATH + "implicit_arg2_vs_null/dev/implicit_arg2_vs_null_clauseArguments.pkl", "rb") as dev_pkl_file:

        train_parse_dict = json.load(train_parse_file)
        dev_parse_dict = json.load(dev_parse_file)
        print("parse dict is loaded...")
        train_clauseArguments = pickle.load(train_pkl_file)
        dev_clauseArguments = pickle.load(dev_pkl_file)
        print("clauseArguments is loaded...")

        # make feature_file file
        trainer.make_feature_file(train_parse_dict, dev_parse_dict, train_clauseArguments, dev_clauseArguments)
        # train
        trainer.train_mode()
        # test
        trainer.test_model()
    ...
    fin.close()
    get_Arg2_acc(dev_parse_dict, dev_relations, dev_feature_path, dev_classifier_out_path)
